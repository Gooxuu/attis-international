...
export const metadata: Metadata = {
  title: "Contact — chauffage, climatisation et bornes de recharge à Montpellier",
  description: `Contactez Attis International à Montpellier : téléphone ${PHONE_DISPLAY}, WhatsApp ou e-mail. ${ADDRESS}.`,
...
const TIPS = [
  ...
  "La nature du projet : chauffage, climatisation, pompe à chaleur ou borne de recharge",
  "Quelques photos de l’emplacement, si possible",
];
      ...
      <section className="bg-gradient-to-b from-mist to-white">
        <Container className="py-16 lg:py-20">
          <SectionHeading
            as="h1"
            eyebrow="Contact"
            title="Contactez Attis International"
            intro="Chauffage, climatisation, pompe à chaleur ou borne de recharge : décrivez-nous votre projet, par le moyen qui vous convient."
          />
        </Container>
      </section>
          ...
          <ul className={`grid gap-6 ${CHANNELS.length === 3 ? "md:grid-cols-3" : "md:grid-cols-2"}`}>
            {CHANNELS.map((channel) => (
              <li key={channel.label}>
                ...
                >
                  <span className="flex size-12 items-center justify-center rounded-2xl bg-amber text-navy-deep">
                    <Icon name={channel.icon} className="size-6" />
                  </span>
                  <p className="mt-5 text-sm font-bold uppercase tracking-wider text-brand-red">{channel.label}</p>
                  <p className="mt-1 break-words font-display text-xl font-extrabold text-navy group-hover:underline">
                    {channel.value}
                  </p>
          ...
          <div className="mt-14 grid gap-8 md:grid-cols-2">
            <div className="rounded-3xl bg-mist p-8">
              <h2 className="flex items-center gap-2 font-display text-xl font-extrabold text-navy">
                <Icon name="pin" className="size-5 text-brand-red" />
                Où nous trouver
              </h2>
