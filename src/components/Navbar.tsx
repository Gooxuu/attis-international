...
export default function Navbar() {
  ...
  const linkClass = (href: string, highlight?: boolean) => {
    ...
    if (highlight) {
      return `inline-flex items-center gap-1.5 rounded-full bg-navy-deep px-4 py-2 text-sm font-semibold text-white ring-1 ring-cyan/70 transition-colors hover:bg-navy ${
        active ? "ring-2" : ""
      }`;
    }
    return `rounded-full px-4 py-2 text-sm font-semibold transition-colors hover:bg-mist ${
      active ? "text-navy underline decoration-amber decoration-2 underline-offset-8" : "text-ink"
    }`;
  };
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-line bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-20 max-w-6xl items-center justify-between gap-4 px-5">
        <Link href="/" aria-label="Attis International — accueil" onClick={() => setOpen(false)}>
        ...
        <nav aria-label="Navigation principale" className="hidden items-center gap-1 lg:flex">
          {NAV_LINKS.map((link) => (
            ...
            >
              {"highlight" in link && link.highlight && <Icon name="bolt" className="size-4 text-cyan" />}
              {link.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2">
          <a
            href={PHONE_TEL}
            className="hidden items-center gap-2 rounded-full bg-amber px-5 py-2.5 text-sm font-bold text-navy-deep transition-colors hover:bg-amber-dark sm:inline-flex"
          >
          ...
          <a
            ...
            className="inline-flex size-11 items-center justify-center rounded-full bg-amber text-navy-deep sm:hidden"
          >
