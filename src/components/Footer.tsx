      ...
      <div className="mx-auto grid max-w-6xl gap-10 px-5 py-14 md:grid-cols-3">
        <div>
          ...
          <p className="mt-4 max-w-xs text-sm leading-relaxed text-white/75">
            Chauffage, climatisation, pompes à chaleur et bornes de recharge pour véhicules électriques, à Montpellier.
          </p>
        </div>

        <div>
          <h2 className="font-display text-sm font-bold uppercase tracking-wider text-amber">Nous contacter</h2>
            ...
            {WHATSAPP_ENABLED && (
              <li>
                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 hover:text-white">
                  <Icon name="chat" className="size-4 text-amber" />
                  WhatsApp
                </a>
              </li>
            )}
        ...
        <div>
          <h2 className="font-display text-sm font-bold uppercase tracking-wider text-amber">Le site</h2>
          <ul className="mt-4 space-y-3 text-sm text-white/85">
            {NAV_LINKS.map((link) => (
              <li key={link.href}>
